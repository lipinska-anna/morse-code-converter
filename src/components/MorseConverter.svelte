<script lang="ts">
    import { converterStore } from '../stores/converterStore.svelte';
    ...
    let outputText = $derived.by(() => {
        const mode = converterStore.isTextToMorse;
        const input = inputText;

        return mode
            ? converterStore.converter.textToMorse(input)
            : converterStore.converter.morseToText(input);
    });
